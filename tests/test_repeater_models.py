"""Tests for RepeaterRequest and SendRecord models."""

from __future__ import annotations

import pytest

from protopoke.replay.models import SendRecord, RepeaterRequest
...
class TestSendRecord:
    ...
    def test_create_failure(self):
        rec = SendRecord.create(b"\x01", b"", "bad-host", 9999, success=False, error="timeout")
        assert not rec.success
        assert rec.error == "timeout"
        assert rec.received_bytes == b""

    def test_serialise_round_trip(self):
        rec = SendRecord.create(b"\xDE\xAD", b"\xBE\xEF", "localhost", 8080, tls=True)
        restored = SendRecord.from_dict(rec.to_dict())
    ...
    def test_null_bytes_round_trip(self):
        rec = SendRecord.create(b"\x00\x00\x00", b"\x00", "h", 1)
        restored = SendRecord.from_dict(rec.to_dict())
        assert restored.sent_bytes == b"\x00\x00\x00"


class TestRepeaterRequest:
    def test_create(self):
        req = RepeaterRequest.create("Tab 1", "10.0.0.1", 443, current_bytes=b"\x01")
    ...
    def test_add_record(self):
        req = RepeaterRequest.create("Tab", "h", 80)
        rec = SendRecord.create(b"\x01", b"\x02", "h", 80)
        req.add_record(rec)
    ...
    def test_serialise_round_trip(self):
        ...
        rec = SendRecord.create(b"\xFF\xFE", b"\x00\x01", "example.com", 443, tls=True)
        req.add_record(rec)

        restored = RepeaterRequest.from_dict(req.to_dict())
    ...
    def test_empty_current_bytes(self):
        req = RepeaterRequest.create("Empty", "h", 80)
        restored = RepeaterRequest.from_dict(req.to_dict())
